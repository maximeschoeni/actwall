Actwall.Filters = class {

  constructor() {

    this.store = {};

    this.filtersAccordeon = new AccordeonItem();
  ...
  async setFilter(key, value) { // -> used by page image-typesl links in header

    this.setParam(key, value);

    await this.renderFilters();
    await this.query();
    if (this.renderHeader) await this.renderHeader();
    await this.renderBody();
  }
  ...
  async query() {
    ...
    this.response = await this.getResults(paramString);

    this.results = this.response.results || this.response; // compat
    ...
    return [
      {
        ...
        update: async (element, implant, render) => {
          ...
          const terms = await this.fetch(`taxonomy/${taxonomy}`);
          this.renderHeaderTaxonomy = render;
          implant.children = terms.map(term => {
            return {
              tag: "li",
              child: {
                tag: "a",
                update: element => {
                  ...
                  const id = this.getParam(taxonomy);
                  element.classList.toggle("active", id === term.id);
                  element.onclick = async event => {
                    event.preventDefault();
                    this.setParam(taxonomy, term.id);
                    await this.renderFilters();
                    await this.query();
                    if (this.renderHeader) await this.renderHeader();
                    await this.renderBody();
    ...
    return {
      ...
      children: [
        ...
        },
        {
          ...
          child: {
            tag: "ul",
            children: this.filters[filter.id].map(item => {
  ...
  buildSearchTab(filter) {
    return {
      class: `filter ${filter.id}-search ${filter.id}`,
      init: (element, implant) => {
      ...
      children: [
        ...
        },
        {
          ...
          child: this.buildSearchInput(filter.id, filter.placeholder)
        }
      ]
    };
  }
      ...
      children: [
        {
          ...
          children: [
            ...
            },
            {
              tag: "span",
              init: element => {
                element.innerHTML = Actwall.__("list a to z");
              }
            }
          ]
        },
          ...
          child: {
            ...
            children: ["a","b","c","d","e","f","g","h","i","j","k","l","m","n","o","p","q","r","s","t","u","v","w","x","y","z"].map(letter => {
              return {
                ...
                update: element => {
                  element.classList.remove("loading");
                  const current = this.getParam("letter") || "";
                  element.classList.toggle("active", current === letter);
                  element.onclick = async event => {
                    event.preventDefault();

                    if (current === letter) {
                      this.removeParam("letter");
                    } else {
                      this.setParam("letter", letter);
                    }
                    element.classList.add("loading");
                    await this.renderFilters();
                    await this.query();
    ...
    return {
      ...
      child: {
        ...
        update: (root, implant, render) => {
          ...
          //     }
          //   ].map(filter => {
          //     return this.buildFilterTab(filter);
          //   }),
          //   ...[
          ...
          //   ].map(filter => {
          //     return this.buildTaxonomyTab(filter);
          //   }),
          //   ...[
          //     {id: "location", name: Actwall.__("Location in Switzerland"), placeholder: Actwall.__("Location")}
          //   ].map(filter => {
          //     return this.buildSearchTab(filter);
          //   })
          // ];
        }
      }
